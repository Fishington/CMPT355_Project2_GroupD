...
// --- ORACLE GENETICALLY TUNED WEIGHTS ---
...
// Fits cleanly into ~6.0 GB of RAM
const int TT_SIZE = 268435456; 
const int LOCK_SIZE = 65536; 

// alignas(64): Prevents CPU Cache "False Sharing"
struct alignas(64) SpinLock {
    std::atomic_flag locked = ATOMIC_FLAG_INIT;
    inline void lock() { 
        while (locked.test_and_set(std::memory_order_acquire)) {
            #if defined(__x86_64__) || defined(_M_X64)
            _mm_pause(); // Hardware optimization for spinlocks
            #endif
        } 
    }
...
struct TranspositionTable {
    ...
    TranspositionTable() {
        // calloc allocates memory instantly via OS memory-mapping
...
// --- TAPERED EVALUATION W/ ORACLE WEIGHTS ---
int evaluate_board(const Board& board, char player) {
    ...
    int pieces_left = __builtin_popcountll(board.b | board.w);
    double phase_factor = (double)pieces_left / 64.0;
    int current_position_weight = std::round(POSITION_WEIGHT * phase_factor);
    int current_mobility_weight = 100 - current_position_weight; 

    int positional_score = 0;
    if (current_position_weight > 0) {
        uint64_t my_pieces = (player == 'B') ? board.b : board.w;
        uint64_t opp_pieces = (player == 'B') ? board.w : board.b;
    ...
    return (mobility_score * current_mobility_weight) + (positional_score * current_position_weight);
}
...
// PASS TT BY REFERENCE: Retains memory persistently!
Move get_best_move(const Board& board, char player, TranspositionTable& global_tt, bool debug_thinking = true) {
    ...
    num_cores = std::min(num_cores, 4u); // Limit to 4 cores to obey rules
    ...
    auto worker_thread = [&](int thread_id) {
        ...
        try {
            ...
            while (!time_out.load(std::memory_order_relaxed)) {
                ...
                if (use_window) {
                    alpha = last_completed_evals[0].second - 500; 
...
// --- MOCK GAME LOOP ---
void play_mock_game() {
    std::cout << "=== KONANE MOCK GAME (16GB/4-CORE OPTIMIZED) ===\n";
    ...
    std::cout << "Allocating memory via OS Anonymous Mapping... Please wait...\n";
    TranspositionTable global_tt;
    std::cout << "Successfully mapped " << format_with_commas(TT_SIZE) << " TT entries into ~6.0 GB of RAM!\n\n";
    
    // We pass a fake hash to apply_move during the mock loop since get_best_move recalculates the root hash anyway.
    ...
    while (true) {
        ...
        if (moves.empty()) {
            std::string winner = (current_player == 'B') ? "White (Random)" : "Black (AI)";
        ...
        Move move;
        if (current_player == 'B') {
            std::cout << "Black (AI) is thinking for " << THINKING_TIME << " seconds...\n";
            move = get_best_move(board, current_player, global_tt, true);
        } else {
        ...
        apply_move(board, move, current_player, fake_hash);
